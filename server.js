...
// v2 Migrations: Add canvas_id columns
try {
  // Check marks table for canvas_id
  const marksColumns = db.prepare("PRAGMA table_info(marks)").all();
  const marksHasCanvasId = marksColumns.some(c => c.name === 'canvas_id');
  if (!marksHasCanvasId) {
    db.exec('ALTER TABLE marks ADD COLUMN canvas_id TEXT DEFAULT NULL REFERENCES canvases(id)');
    console.log('Migration: Added canvas_id column to marks table');
  }
  
  // Check evolution_log table for canvas_id
  const evolutionColumns = db.prepare("PRAGMA table_info(evolution_log)").all();
  const evolutionHasCanvasId = evolutionColumns.some(c => c.name === 'canvas_id');
  if (!evolutionHasCanvasId) {
    db.exec('ALTER TABLE evolution_log ADD COLUMN canvas_id TEXT DEFAULT NULL REFERENCES canvases(id)');
    console.log('Migration: Added canvas_id column to evolution_log table');
  }
  
  // Check agents table for canvas_id and subtheme
  const agentsColumns = db.prepare("PRAGMA table_info(agents)").all();
  const agentsHasCanvasId = agentsColumns.some(c => c.name === 'canvas_id');
  const agentsHasSubtheme = agentsColumns.some(c => c.name === 'subtheme');
  if (!agentsHasCanvasId) {
    db.exec('ALTER TABLE agents ADD COLUMN canvas_id TEXT DEFAULT NULL REFERENCES canvases(id)');
    console.log('Migration: Added canvas_id column to agents table');
  }
  if (!agentsHasSubtheme) {
    db.exec('ALTER TABLE agents ADD COLUMN subtheme TEXT DEFAULT NULL');
    console.log('Migration: Added subtheme column to agents table');
  }
} catch (e) {
